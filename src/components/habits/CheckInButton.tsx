...
export function CheckInButton({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.92 }}
      aria-label={checked ? "Uncheck habit" : `Check in (${currentCount}/${targetCount})`}
    >
      ...
      <motion.div
        ...
        animate={{
          backgroundColor: checked ? color : partial ? `${color}18` : cardHovered ? `${color}18` : "rgba(255,255,255,0.04)",
        }}
        transition={{ duration: 0.3 }}
      />

      {/* Content: check icon when complete, count when partial, empty otherwise */}
